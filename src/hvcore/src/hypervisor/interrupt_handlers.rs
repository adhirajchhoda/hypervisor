...
impl InterruptDescriptorTable {
    ...
    pub fn clone_from_current() -> Self {
        ...
        let mut idt = zeroed_box::<InterruptDescriptorTableRaw>();
        let src = idtr.base as *const InterruptDescriptorTableEntry;
        let count =
            (usize::from(idtr.limit) + 1) / core::mem::size_of::<InterruptDescriptorTableEntry>();
        let count = count.min(0x100);
        unsafe {
            core::ptr::copy_nonoverlapping(src, idt.0.as_mut_ptr(), count);
        }

        Self { ptr: idt }
    }

    pub fn set_ist(&mut self, vector: usize, ist_index: u8) {
        assert!(vector < 0x100);
        assert!(ist_index <= 7);
        self.ptr.0[vector].ist = ist_index;
    }
