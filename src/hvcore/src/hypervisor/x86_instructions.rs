...
pub(crate) fn ensure_sse_enabled() {
    unsafe {

        let mut cr0_val: u64;
        asm!("mov {}, cr0", out(reg) cr0_val, options(nomem, nostack, preserves_flags));
        cr0_val &= !(1u64 << 3);
        cr0_val &= !(1u64 << 2);
        cr0_val |= 1u64 << 1;
        asm!("mov cr0, {}", in(reg) cr0_val, options(nomem, nostack));

        let mut cr4_val: u64;
        asm!("mov {}, cr4", out(reg) cr4_val, options(nomem, nostack, preserves_flags));
        cr4_val |= 1u64 << 9;
        cr4_val |= 1u64 << 10;
        asm!("mov cr4, {}", in(reg) cr4_val, options(nomem, nostack));
    }
}
